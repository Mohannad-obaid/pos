...
import '../../../providers/repository_providers.dart'; // مسار قاعدة البيانات للوصول لكلاس Product
...
class _ProductsListScreenState extends ConsumerState<ProductsListScreen> {
  ...
  @override
  Widget build(BuildContext context) {
    // جلب قائمة المنتجات الحية من قاعدة البيانات
    ...
    return Scaffold(
      ...
      body: productsAsync.when(
        ...
        data: (allProducts) {
          ...
          // 1. استخراج التصنيفات الفريدة من المنتجات الموجودة
          final categories = ['الكل', ...allProducts.map((p) => p.category).toSet()];

          // 2. تطبيق الفلترة (حسب البحث والتصنيف)
          var filteredProducts = allProducts.where((p) {
            // فلتر البحث (بالاسم أو الباركود)
              ...
              Expanded(
                child: filteredProducts.isEmpty
                  ...
                  child: Text(
                    ...
                    style: TextStyle(fontSize: 16.sp, color: AppColors.outline),
                  ),
                )
                    : ListView.builder(
                  padding: EdgeInsets.only(left: 16.w, right: 16.w, top: 16.h, bottom: 100.h),
                  ...
                  itemBuilder: (context, index) {
                    ...
                    // تحديد حالة التوفر
                    final isAvailable = !product.trackStock || product.stockQuantity > 0;
                    ...
                    return _buildProductItem(
                      ...
                      icon: Icons.inventory_2_outlined, // يمكن تغييره حسب التصنيف لاحقاً
                      isAvailable: isAvailable,
                      onTap: () {

                        NavigationService.navigateTo(AppRoutes.productDetails,arguments: ProductDetailsArgs(productId: product.id) );
                        // هنا يمكنك تمرير بيانات المنتج إلى شاشة التفاصيل إذا أردت
                        if (isAvailable) {
                          // إضافة المنتج للسلة عند الضغط
      ...
      floatingActionButton: Padding(
        ...
        child: SizedBox(
          ...
          child: FloatingActionButton.extended(
            onPressed: () {
              NavigationService.navigateTo(AppRoutes.scanBarcode);
            },
            backgroundColor: AppColors.primary,
            elevation: 4,
            icon: Icon(Icons.qr_code_scanner, color: AppColors.onPrimary),
          ...
          Padding(
            ...
            child: Container(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  hintText: 'ابحث عن منتج أو باركود...',
                  hintStyle: TextStyle(fontSize: 14.sp, color: AppColors.outline),
                  prefixIcon: Icon(Icons.search, color: AppColors.outline),
      ...
      child: Container(
        ...
        child: Row(
          children: [
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Icon(Icons.qr_code, size: 14.sp, color: AppColors.outline),
                      SizedBox(width: 4.w),
                      Text(barcode, style: TextStyle(fontSize: 10.sp, color: AppColors.outline, letterSpacing: 1.1 ) ),
                    ],
                  ),
                ],
              ),
            ...
            SizedBox(width: 8.w),
            Icon(Icons.add_shopping_cart, color: isAvailable ? AppColors.primary : AppColors.outline, size: 20.sp),
          ],
        ),
      ),
    );
