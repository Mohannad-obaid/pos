...
import 'package:pos/providers/cart_provider.dart' show cartProvider, cartItemsCountProvider, cartTotalProvider;
import '../../../core/theme/app_colors.dart';

// 1. تحويل الكلاس إلى ConsumerWidget
class CartScreen extends ConsumerWidget {
  const CartScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // 2. الاستماع لقائمة المنتجات في السلة
    final cartItems = ref.watch(cartProvider);

    return Scaffold(
      backgroundColor: AppColors.surface,
      appBar: _buildAppBar(context, ref, cartItems.length),
      body: Column(
        children: [
          Expanded(
            child: cartItems.isEmpty
                ? Center(
              child: Text('السلة فارغة', style: TextStyle(fontSize: 18.sp, color: AppColors.outline)),
            )
                : ListView.separated(
              padding: EdgeInsets.symmetric(horizontal: 16.w, vertical: 16.h),
          ...
          // إظهار الملخص السفلي فقط إذا كان هناك منتجات
          if (cartItems.isNotEmpty) _buildBottomSummary(context, ref),
        ],
      ),
    );
  }

  PreferredSizeWidget _buildAppBar(BuildContext context, WidgetRef ref, int itemsCount) {
    return AppBar(
      backgroundColor: AppColors.surfaceContainerLowest,
      elevation: 0,
      scrolledUnderElevation: 0,
  ...
  Widget _buildCartItem(BuildContext context, WidgetRef ref, dynamic item) {
    ...
    return Dismissible(
      ...
      onDismissed: (direction) {
        // حذف المنتج من السلة عبر Riverpod
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(product.name, style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.w600, color: AppColors.onSurface)),
                  Text('₪ ${product.price.toStringAsFixed(2)}', style: TextStyle(fontSize: 13.sp, color: AppColors.onSurfaceVariant)),
                ],
              ),
            ),
            Text(
  ...
  Widget _buildBottomSummary(BuildContext context, WidgetRef ref) {
    // 3. الاستماع للإجمالي المحسوب تلقائياً
            ...
            Row(
              ...
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('$itemsCount منتجات · قبل الضريبة: ₪ ${total.toStringAsFixed(2)}',
                        style: TextStyle(fontSize: 12.sp, color: AppColors.onSurfaceVariant)),
                    SizedBox(height: 4.h),
                    Text('المجموع:', style: TextStyle(fontSize: 20.sp, fontWeight: FontWeight.bold, color: AppColors.onSurface)),
                  ],
              ...
              children: [
                ...
                Expanded(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () {
                      // التوجيه إلى شاشة الدفع (CheckoutScreen)
