...
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:drift/drift.dart' as drift; // لاستخدام Value و InvoicesCompanion
import '../../../core/theme/app_colors.dart';
...
class QuickDebtEntryScreen extends ConsumerStatefulWidget {
  const QuickDebtEntryScreen({super.key});

  @override
  ConsumerState<QuickDebtEntryScreen> createState() => _QuickDebtEntryScreenState();
}

class _QuickDebtEntryScreenState extends ConsumerState<QuickDebtEntryScreen> {
  String _amount = "0";
  Customer? _selectedCustomer; // العميل المحدد حالياً
  ...
  // دالة لحفظ الدين الحقيقي في قاعدة البيانات
  Future<void> _saveDebt() async {
    ...
    try {
      // إنشاء فاتورة "دين" فارغة المنتجات
      final invoiceCompanion = InvoicesCompanion.insert(
        customerId: drift.Value(_selectedCustomer!.id),
        totalAmount: amountValue,
        date: DateTime.now(),
        status: 'دين',
        // إذا كان هناك حقل للملاحظات في الفاتورة يمكنك تمريره هنا، مثال:
        // note: drift.Value(_noteController.text),
      );

      // استدعاء دالة إنشاء الفاتورة (isDebt = true لكي يضيف المبلغ لرصيد العميل)
      await ref.read(invoiceRepositoryProvider).createInvoice(invoiceCompanion, [], true);

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('تم تسجيل الدين بنجاح!'), backgroundColor: Colors.green),
        );
        Navigator.pop(context); // إغلاق الشاشة بعد الحفظ
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  // نافذة سفلية لاختيار عميل من القائمة
        ...
        return SafeArea(
          child: Column(
            ...
            children: [
              Padding(
                padding: EdgeInsets.all(16.w),
                child: Text('اختر عميلاً', style: TextStyle(fontSize: 18.sp, fontWeight: FontWeight.bold)),
              ),
      ...
      children: [
        ...
        SizedBox(
          ...
          child: customersAsync.when(
            ...
            data: (customers) {
              if (customers.isEmpty) return Text('لا يوجد عملاء مسجلين', style: TextStyle(color: AppColors.outline));

              // نأخذ أول 4 عملاء فقط لتصميم الـ Chips
              final recentCustomers = customers.take(4).toList();

              return ListView(
                scrollDirection: Axis.horizontal,
                children: recentCustomers.map((c) => _recentCustomerChip(c)).toList(),
              );
            },
          ),
        ),
  ...
  Widget _recentCustomerChip(Customer customer) {
    return Container(
      margin: EdgeInsets.only(left: 8.w),
      child: ActionChip(
        label: Text(customer.name.split(' ').first, style: TextStyle(fontSize: 14.sp, color: AppColors.onSurface)), // عرض الاسم الأول فقط
        backgroundColor: AppColors.surfaceContainerLow,
        side: BorderSide(color: AppColors.outlineVariant, width: 0.5),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20.r)),
        onPressed: () {
  ...
  Widget _buildCustomerSelector() {
    // تحديد ما سيتم عرضه بناءً على اختيار العميل
    final String displayName = _selectedCustomer?.name ?? 'اختر عميلاً';
    final String firstLetter = _selectedCustomer != null ? displayName.substring(0, 1) : '؟';
    final Color avatarColor = _selectedCustomer != null ? AppColors.primaryFixed : AppColors.surfaceVariant;
    final Color textColor = _selectedCustomer != null ? AppColors.onPrimaryFixed : AppColors.onSurfaceVariant;
            ...
            Row(
              children: [
                CircleAvatar(
                  ...
                  child: Text(firstLetter, style: TextStyle(color: textColor, fontWeight: FontWeight.bold, fontSize: 16.sp)),
                ),
                SizedBox(width: 12.w),
                Text(displayName, style: TextStyle(fontSize: 18.sp, fontWeight: FontWeight.w600, color: AppColors.onSurface)),
              ],
            ),
            Icon(Icons.expand_more, color: AppColors.onSurfaceVariant),
          ],
  ...
  Widget _buildSaveButton() {
    return Container(
      ...
      child: ElevatedButton(
        ...
        onPressed: _isSaving ? null : _saveDebt, // استدعاء دالة الحفظ
        child: _isSaving
            ? const CircularProgressIndicator(color: AppColors.onPrimaryContainer)
            : Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Text('حفظ الدين', style: TextStyle(fontSize: 20.sp, fontWeight: FontWeight.bold, color: Colors.white)),
            SizedBox(width: 8.w),
            Icon(Icons.check_circle_outline, size: 24.sp,color: Colors.white),
          ],
        ),
      ),
    );
